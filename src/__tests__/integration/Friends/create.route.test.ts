...
describe("Add friends route tests", () => {
  let conn: DataSource;
  ...
  it("Should be able to add a friend", async () => {
    ...
    const expectedResults = {
      status: 201,
      bodyToEqual: { message: "friend add" },
    };
  ...
  it("Should not be able to add a friend with missing token", async () => {
    const listUser1 = userRepo.create(mockedUsersListRequest[0]);
    await userRepo.save(listUser1);
    const user1 = await userRepo.findOneBy({ nick: "nickTest1" });
  ...
  it("Should not be able to add a friend if nick not exists", async () => {
    const listUser1 = userRepo.create(mockedUsersListRequest[0]);
    await userRepo.save(listUser1);
    const user1 = await userRepo.findOneBy({ nick: "nickTest1" });
